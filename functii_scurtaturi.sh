...
alias gt-accounts='cd $HOME/projects/Dataglide/Publish.Config/takeon-import/Create\ Accounts\ with\ LEI'
...
# Takes config.zip generated by the ingress generator tool and
# - unzip it 
# - makes the Resources directory
# - puts a Resources dir inside unziped folder and copy the original file
# - cleans unused files
#
# The script starts with checking that few conditions are met:
# - Only one file in the pipeline folder exists and the extension is not zip
# - Only one file with zip extension that starts with 'config...' exists in the Downloads folder
# 
# Obs:
# path to Downloads dir and pipeline dir are hardcoded here
function put_ingres_config() {
	download_folder="$HOME/Downloads"
	pipeline_folder="$HOME/Downloads/pipeline"

	# Check if pipeline folder exist. If not, make it
	if [[ ! -d $pipeline_folder ]]; then
		mkdir $pipeline_folder
	fi


	# Check if only one file exists inside the pipeline folder
	if [[ ! 1 -eq $(ls $pipeline_folder | wc -l) ]]; then
		echo "Aborted: Only the original input file should be inside ~/Downloads/pipeline/ directory"
		return 1
	fi

	# Check that the file in pipeline folder don't have zip extension
	if [[ 0 -ne $(find $pipeline_folder -maxdepth 1 -type f | grep /*.zip | wc -l) ]];then
		echo "Aborted: The original input file inside ~/Downloads/pipeline/ directory cannot have zip extension"
		return 1
	fi

	# Check there is at least one zip file in Downloads folder
	if [[ 0 -eq $(find ~/Downloads/ -maxdepth 1 -type f | grep /*.zip | wc -l) ]];then
		echo "Aborted: A zip config file should be in the Downloads folder. Currently there is none "
		return 1
	fi

	# Check if only one file in Downloads folder with zip extension
	if [[ 1 -ne $(find ~/Downloads/ -maxdepth 1 -type f | grep /*.zip | wc -l) ]];then
		echo "Aborted: Only one zip file should be in Downloads directory. Currently: "
		find $download_folder/ -maxdepth 1 -type f | grep /*.zip
		return 1
	fi

	# Check that the name of the zip file in Downloads folder starts with config
	if [[ 1 -ne $(find ~/Downloads/ -maxdepth 1 -type f -name "config*.zip" | wc -l) ]];then
		printf "Aborted: Format of zip config file in Downloads folder should have the form confing_[client_name].zip. \nCurrently:\n"
		find $download_folder -maxdepth 1 -type f -name "*.zip"
		return 1
	fi

	# Delete old config zip file if exists
	if [[ -f $download_folder/config.zip ]];then
		rm $download_folder/config.zip
	fi

	# Rename to config.zip
	cp $download_folder/config*.zip $download_folder/config.zip

	read -p "[?] Client name: " CLIENT_NAME

	# Unzip and remove zip file
	mv $download_folder/config.zip $pipeline_folder/
	unzip -d $pipeline_folder $pipeline_folder/config.zip
	rm $pipeline_folder/config.zip

	# Rename to client name
	mv $pipeline_folder/config*/ $pipeline_folder/config_$CLIENT_NAME/

	# Make Resources folder inside
	mkdir $pipeline_folder/"config_$CLIENT_NAME"/Resources/
